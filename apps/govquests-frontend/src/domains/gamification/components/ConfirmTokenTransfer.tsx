...
export default function ConfirmTokenTransfer() {
  ...
  const { data: rewardIssuanceData, isLoading: isLoadingRewardIssuance } =
    useRewardIssuance(poolId, userId);
  ...
  if (!rewardIssuanceData?.rewardIssuance) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-lg">No reward issuance found</div>
      </div>
    );
  }
